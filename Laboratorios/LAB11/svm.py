...
test_images = np.array([plt.imread(i) for i in glob.glob
...
print('No. imagenes de vehiculos: ', n_car_images)
print('No. imagenes de no vehiculos: ', n_non_car_images)
print('Imagen Dimension: ', img_shape)
...
for i in np.arange(32):
    img = cv2.imread(non_car_images[np.random.randint
    					(0,n_non_car_images)])
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    axs[i].axis('off')
    axs[i].imshow(img)
...
def get_hog_features(img, orient, pix_per_cell, cell_per_block, 
    ...
    # Otherwise call with one output
        ...
        features = hog(img, orientations=orient, 
        	pixels_per_cell=(pix_per_cell, pix_per_cell),
            cells_per_block=(cell_per_block, cell_per_block),
            transform_sqrt=True, visualize=vis,
...
titles = []
for i in range(3):
    for feature_image, img_type in zip([car_test, non_car_test], 
    			['Car', 'Non-car']):
        channel = feature_image[:,:,i]
        imgs.append(channel)
        titles.append(img_type + ' CH%d' % i)
        features, hog_image = get_hog_features(channel, orient=12, 
        			pix_per_cell=8, cell_per_block=2, 
...
car_features = extract_features(car_images, cspace=colorspace,
			orient=orient, 
            pix_per_cell=pix_per_cell, cell_per_block=cell_per_block, 
            hog_channel=hog_channel, spatial_size=(spatial, spatial),
           	hist_bins=hist_bins, hist_range=(0, 256))

non_car_features = extract_features(non_car_images,cspace=colorspace,
			orient=orient, 
            pix_per_cell=pix_per_cell, cell_per_block=cell_per_block, 
            hog_channel=hog_channel, spatial_size=(spatial, spatial),
            hist_bins=hist_bins, hist_range=(0, 256))
...
print('Using spatial binning of:',spatial,'and', 
...
print('Test Accuracy of SVC = ', round(svc.score(
				X_test, y_test), 4))
...
t=time.time()
n_predict = 10
print('My SVC predicts:     ', svc.predict(
				X_test[0:n_predict]))
...
def find_cars(img, svc, X_scaler, orient, pix_per_cell, cell_per_block, 
		spatial_size, hist_bins, ystart_ystop_scale, h_shift=0,
		visualisation=False):
        ...
        for xb in range(nxsteps):
            for yb in range(nysteps):
                ...
                if(visualisation):
                    ...
                    box_vis.append(((xbox_left, ytop_draw+ystart),
                ...
                if test_prediction == 1:
                    ...
                    win_draw = np.int(window*scale)
                    # Append Detection Position to list 
                    bbox_detection_list.append(((xbox_left+h_shift, 
                    	ytop_draw+ystart),
                    	(xbox_left+win_draw+h_shift,ytop_draw+win_draw+ystart)))
                    # Draw Detection on image
                    cv2.rectangle(draw_img,(xbox_left+h_shift, 
                    	ytop_draw+ystart),
                    	(xbox_left+win_draw+h_shift,ytop_draw+win_draw+ystart),
                    	(0,0,255),6) 
...
ystart_ystop_scale = [(405, 510, 1), (400, 600, 1.5),
				(415, 710, 2.5)]
bbox_detection_list, detections, box_vis_list = find_cars(test_image_sliding,
				svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size,
				hist_bins, ystart_ystop_scale, h_shift=500, visualisation=True)
...
title = "Escala: 0.9 "  + " Cantidad: " + str(len(box_vis_list[0])) 
ax1.set_title(title, fontsize=20)
ax1.axis("off")
ax2.imshow(windows_img[1])
title = "Escala: 1.4"  + " Cantidad: " + str(len(box_vis_list[1])) 
ax2.set_title(title, fontsize=20)
ax2.axis("off")
ax3.imshow(windows_img[2])
title = "Escala: 2.3"  + " Cantidad: " + str(len(box_vis_list[2])) 
...
project_video_res = './dataset/project_video_result.mp4'
clip1 = VideoFileClip("./dataset/project_video.mp4")
